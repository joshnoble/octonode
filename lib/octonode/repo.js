...
(function() {
  ...
  Repo = (function() {
    ...
    Repo.prototype.subscribers = function() {
      var cb, page, params, per_page, _i;
      params = 2 <= arguments.length ? __slice.call(arguments, 0, _i = arguments.length - 1) : (_i = 0, []), cb = arguments[_i++];
      page = params[0] || 1;
      per_page = params[1] || 30;
      return this.client.get("/repos/" + this.name + "/subscribers", page, per_page, function(err, s, b, headers) {
        if (err) {
          return cb(err);
        }
        if (s !== 200) {
          return cb(new Error("Repo subscribers error"));
        } else {
          return cb(null, b, headers);
        }
      });
